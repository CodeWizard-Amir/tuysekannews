...
<section class="w-full lg:w-3/4 mx-auto my-5 p-5">
    ...
    <div class="w-full flex my-8 flex-col">
        ...
        @if (!$news_result->count() > 0)
            <p class="my-5 py-3 border-r-2 border-amber-500 bg-amber-50 text-amber-700 px-5">خبری با این عنوان یافت نشد!
            </p>
        @endif
        @foreach ($news_result as $nr)
            <a href="{{ route('websitepages.EachNews', ['newsID' => $nr?->newsID, 'title' => str_replace(' ', '-', $nr->title)]) }}"
                class="w-full flex h-[170px] shadow-md border">
    ...
    <div class="w-full flex my-8 flex-col">
        ...
        <div class="w-full flex justify-between flex-wrap my-3">
            @if (!$celebrity_result->count() > 0)
                <p class="my-5 py-3 border-r-2 w-full border-amber-500 bg-amber-50 text-amber-700 px-5">مشاهیری با این
                    عنوان یافت نشد!</p>
            @endif
    ...
    <div class="w-full flex my-8 flex-col">
        ...
        <div class="w-full flex justify-between flex-wrap my-3">
            @if (!$work_result->count() > 0)
                <p class="my-5 py-3 border-r-2 border-amber-500 bg-amber-50 text-amber-700 px-5">آثاری با این عنوان یافت
                    نشد !</p>
            @endif
